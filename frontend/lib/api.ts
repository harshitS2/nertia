...
// ---------------------------------------------------------------------------
// Type: Registration form payload
// ---------------------------------------------------------------------------
export interface RegisterFormPayload {
  ...
  phone:   string;
  program: string;
  status:  string; // e.g. "Student" | "Fresher" | "Working professional"
}

// ---------------------------------------------------------------------------
// submitRegisterForm
//
// POSTs candidate registration data to the Express backend.
// ---------------------------------------------------------------------------
export async function submitRegisterForm(data: RegisterFormPayload): Promise<void> {
  ...
  try {
    ...
    if (process.env.NODE_ENV === "development") {
      console.warn(
        "[api.ts] Backend not reachable. Simulating registration success for local dev.",
        networkError
      );
      await new Promise((resolve) => setTimeout(resolve, 800));
      return;
